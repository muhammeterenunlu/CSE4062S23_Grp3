...
def c_svm(data):
    ...
    evaluation_table.loc[0] = [1, 'C-SVM', accuracy, f1_macro, None, auc, None]
...
def nu_svm(data):
    ...
    svm_model = NuSVC(kernel='rbf', nu=0.5, gamma='scale')
    ...
    # Create a table to store the evaluation metrics
    evaluation_table = pd.DataFrame(columns=['#', 'Experiment', 'Accuracy', 'F1-macro', 'F1-micro', 'AUC', 'SSE'])
    evaluation_table.loc[0] = [1, 'Nu-SVM', accuracy, f1_macro, None, auc, None]
...
def one_class_svm(data):
    ...
    svm_model = OneClassSVM(kernel='rbf', nu=0.5, gamma='scale')
    ...
    # Create a table to store the evaluation metrics
    evaluation_table = pd.DataFrame(columns=['#', 'Experiment', 'Accuracy', 'F1-macro', 'F1-micro', 'AUC', 'SSE'])
    evaluation_table.loc[0] = [1, 'One-Class SVM', accuracy, f1_macro, None, auc, None]
...
# Call the SVM functions with your dataset
c_svm(data)
nu_svm(data)
one_class_svm(data)
